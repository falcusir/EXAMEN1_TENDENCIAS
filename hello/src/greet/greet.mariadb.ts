...
//Guarda esta conexión en la variable Connection
...
//Busca todos los saludos en la tabla regards y los devuelve.
 ...
 //Busca un saludo específico usando su ID.
  ...
  static async delete(id: number) {
    const result = await connection.query(
      'DELETE FROM regards WHERE id = ?', [id]
    )
  ...
  static async stats() {
    // Total de registros
    const totalQuery = await connection.query(
      'SELECT COUNT(*) AS total FROM regards'
    )

    // Agrupación por idioma
    const groupQuery = await connection.query(
